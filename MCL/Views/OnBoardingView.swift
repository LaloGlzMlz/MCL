...
struct OnBoardingView: View {
    var onBoardingItem: [OnBoardinItem] = onBoardingData
    ...
    var body: some View {
        NavigationView {
            VStack {
                ...
                HStack {
                    Spacer()
                    NavigationLink (destination: AlbumShelfView().navigationBarBackButtonHidden()
                        .onAppear {
                            isOnboarded = true
                        })
                    {
                        Capsule()
                            .fill(Color(hex: "E4255B"))
